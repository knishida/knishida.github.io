...
1. 勝見 久央 (NTT), 水上 雅博 (NTT), 神山 歩相名 (NTT), 西田 京介 (NTT), Chain-of-Argumentation: LLMエージェントによる論証構築のための情報探索戦略, NLP若手の会 (YANS) 第19回シンポジウム (2025), S1-P22, June 2025.

1. 川﨑 春佳 (NTT), 壹岐 太一 (NTT), 田中 涼太 (NTT), 西田 京介 (NTT), 文書画像理解における大規模視覚言語モデルの内部表現は回答に反映されているのか？, NLP若手の会 (YANS) 第19回シンポジウム (2025), S2-P21, June 2025.

1. 大賀 悠平 (NTT), 大塚 淳史 (NTT), 有本 庸浩 (NTT), 水上 雅博 (NTT),  RAGシステムにおけるビジネス文書処理能力の評価用データセットの構築, NLP若手の会 (YANS) 第19回シンポジウム (2025), S5-P19, June 2025.

1. 岡 佑依, 長谷川 拓, 西田 京介, 齋藤 邦子 (NTT)P2-4	ウェーブレット位置符号化, 言語処理学会第31回年次大会（NLP2025）, March 2025. **優秀賞（14/765件）**

1. 西田 光甫, 西田 京介, 齋藤 邦子 (NTT) C10-5	大規模言語モデルの再パラメタ化に基づく初期化による損失スパイクの抑制, 言語処理学会第31回年次大会（NLP2025）, March 2025. **優秀賞（14/765件）**

1. 田中 涼太 (NTT/東北大), 壹岐 太一, 長谷川 拓, 西田 京介, 齋藤 邦子 (NTT), 鈴木 潤 (東北大) C9-4	VDocRAG: 視覚的文書に対する検索拡張生成, 言語処理学会第31回年次大会（NLP2025）, March 2025., **委員特別賞（32/726件）**

1. 篠田 一聡, 北条 伸克, 西田 京介, 水野 沙希, 鈴木 啓太, 増村 亮, 杉山 弘晃, 斎藤 邦子 (NTT) B3-6	ToMATO: 心の理論ベンチマークのためのロールプレイングLLMの心的状態の言語化, 言語処理学会第31回年次大会（NLP2025）, March 2025.

1. 有山 知希 (東北大), 鈴木 潤 (東北大/理研), 鈴木 正敏 (Studio Ousia), 田中 涼太 (NTT/東北大), 赤間 怜奈 (東北大/理研), 西田 京介 (NTT) Q7-23J	クイズコンペティションの結果分析から見た日本語質問応答の到達点と課題, 言語処理学会第31回年次大会（NLP2025）, March 2025. **2024年言語処理学会論文賞 (1/22件)**

1. 門谷 宙 (NTT), 西田 光甫 (NTT), 西田 京介 (NTT), 齋藤 邦子 (NTT), In-model anti-expertによる大規模言語モデルのハルシネーション抑制, NLP若手の会 (YANS) 第18回シンポジウム (2024), S2-P20, September 2024.
   
1. 田中涼太 (NTT/東北大), 壱岐太一, 西田京介, 齋藤邦子 (NTT), 鈴木潤 (東北大)  C3-4	InstructDoc: 自然言語指示に基づく視覚的文書理解, 言語処理学会第30回年次大会（NLP2024）, March 2024. **優秀賞（12/599件）**

1. 西田光甫 (NTT/東大), 吉永直樹 (東大), 西田京介 (NTT) クロスドメイン検索型質問応答のためのニューラル疎ベクトル検索, 言語処理学会第30回年次大会（NLP2024）, C1-3, March 2024.
   
1. 門谷宙, 西田光甫, 西田京介, 齋藤邦子 (NTT)	A2-1	大規模言語モデルは自身の Hallucination を検知できるか?, 言語処理学会第30回年次大会（NLP2024）, March 2024.
   
1. 大賀悠平, 長谷川拓, 西田京介, 齋藤邦子 (NTT) C8-3	LLMを用いた不適切発話データの自動生成に関する研究, 言語処理学会第30回年次大会（NLP2024）, March 2024.
